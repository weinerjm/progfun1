 ...
 */
 @RunWith(classOf[JUnitRunner])
  ...
  test("max throws error on empty list") {
  ...
  // max: negatives and zero
  test("max of non-positive numbers") {
    assert(max(List(-1, -10, 0)) === 0)
  }
  ...
  test("max with multiple same entries") {
    assert(max(List(2, 1, 2)) === 2)
  }
}
