...
object Huffman {
   ...
   */
    def until(singlef: List[CodeTree] => Boolean, combf: List[CodeTree] => List[CodeTree])(trees: List[CodeTree]): List[CodeTree] = {
      if (singlef(trees)) trees
      else until(singlef, combf)(combf(trees))
    }
   ...
   */
    def createCodeTree(chars: List[Char]): CodeTree = {
      // first get the sorted list of (char, times)
      val timesList = times(chars)
      val orderedLeaves = makeOrderedLeafList(timesList)
   ...
   */
    def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
      def decodeHelper(subtree: CodeTree, bits: List[Bit]): List[Char] = subtree match {
        case subtree: Leaf => if (bits.isEmpty) List(subtree.char) else subtree.char :: decodeHelper(tree, bits)
        case subtree: Fork => if (bits.head == 0) decodeHelper(subtree.left, bits.tail) else decodeHelper(subtree.right, bits.tail)
      }
   ...
   */
    def convert(tree: CodeTree): CodeTable = tree match {
      ...
      case tree: Fork => mergeCodeTables(convert(tree.left), convert(tree.right))
    }
   ...
   */
    def mergeCodeTables(a: CodeTable, b: CodeTable): CodeTable = {
      def mergeHelper(b: Int, pair: (Char, List[Bit])): (Char, List[Bit]) = pair match {
        case (char, bits) => (char, b :: bits)
      }
      a.map(mergeHelper(0, _)) ::: b.map(mergeHelper(1, _))
    }
